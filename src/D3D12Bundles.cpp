...
void D3D12Bundles::LoadAssets()
{
	...
	// Create the root signature.
	{
		...
		ranges[3].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 1, 0);  //t1

		CD3DX12_ROOT_PARAMETER1 rootParameters[4];
		rootParameters[0].InitAsDescriptorTable(1, &ranges[0], D3D12_SHADER_VISIBILITY_PIXEL);
		rootParameters[1].InitAsDescriptorTable(1, &ranges[1], D3D12_SHADER_VISIBILITY_PIXEL);
		rootParameters[2].InitAsDescriptorTable(1, &ranges[2], D3D12_SHADER_VISIBILITY_ALL);
		rootParameters[3].InitAsShaderResourceView(1, 0); //t1
	...
	// Create the vertex buffer.
	{
		...
		auto createBoxGeometry = [&](float width, float height, float depth)
		{
			//MeshData meshData;
		...
		createBoxGeometry(0.1f, 0.1f, 0.1f);

		unsigned int verticesSize = (unsigned int)Vertices.size() * sizeof(Vertex);
		unsigned int indicesSize = (unsigned int)Indices32.size() * sizeof(unsigned int);
		...
		indexData.pData = Indices32.data();
		...
		m_numIndices = indicesSize / 4;    // R32_UINT (SampleAssets::StandardIndexFormat) = 4 bytes each.
		//m_numIndices = (unsigned int)Indices32.size();
	}
	...
	CreateFrameResources(m_instances);
}
...
void D3D12Bundles::OnUpdate()
{
	...
	m_pCurrentFrameResource->UpdateInstanceData(m_instances);
}
...
void D3D12Bundles::CreateFrameResources(std::vector<InstanceData>& instances)
{
	...
	for (UINT i = 0; i < FrameCount; i++)
	{
		FrameResource* pFrameResource = new FrameResource(m_device.Get(), instances);


		//TODO why removing this causes a blink
		UINT64 cbOffset = 0;
...
void D3D12Bundles::BuildRenderIdems()
{
	//define signal slots
	SignalSlot topSlot;
	topSlot.RelativePosition = { -7.f, 34.f };

	SignalSlot middleSlot;
	middleSlot.RelativePosition = { -7.f, 12.f };

	SignalSlot bottomSlot;
	bottomSlot.RelativePosition = { -7.f, -5.f };

	SignalPlayground playground;
	playground.Play(topSlot, middleSlot, bottomSlot);

	DrawSignalSlot(topSlot);
	DrawSignalSlot(middleSlot);
	DrawSignalSlot(bottomSlot);
}

void D3D12Bundles::DrawSignalSlot(SignalSlot& slot)
{
	//items per second
	const unsigned int itemsPerSecond = 1000;
	const float secondWidth = 20.f;
	const float itemDistance = (float)secondWidth / itemsPerSecond;
	const unsigned int numInstances = slot.size();
	const unsigned int totalSeconds = numInstances / itemsPerSecond;
	//float timeStep = (float)totalSeconds / numInstances;

	unsigned int instanceOffset = (unsigned int)m_instances.size();
	unsigned int numRenderItems = slot.size();

	m_instances.resize(instanceOffset + numRenderItems);

	for (unsigned int index = 0; index < slot.size(); ++index)
	{
		float x = index * itemDistance;

		float y = 0.f;
		float z = 0.f;

		y += slot.evaluate(index);
		z += slot.evaluate2(index);
		x += slot.RelativePosition.first;
		y += slot.RelativePosition.second;

		m_instances[instanceOffset + index].Position = { x, y, z, 0.f };
